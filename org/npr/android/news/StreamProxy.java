...
public class StreamProxy implements Runnable {
	...
	private SongParserClient mClient = null;
	...
	public void run() {
		...
		while (isRunning) {
			...
			try {
				...
				do{
					restart = processRequest(request, client);
				}while(restart == true);
				

			} catch (SocketTimeoutException e) {
				// Do nothing
				Log.e(LOG_TAG, "Error timed out", e);
	...
	private void sendMetaString(String metaString) throws IOException {
	...
	private boolean processRequest(HttpRequest request, Socket client)
		...
		for (Header h : response.getAllHeaders()) {
			httpString.append(h.getName()).append(": ").append(h.getValue())
					.append("\r\n");
			if (h.getName().compareTo("icy-metaint") == 0) {
				metaDataInterval = Integer.parseInt(h.getValue());
			}
		}
		httpString.append("\r\n");
		
		if(httpString.indexOf("Service Unavailable") != -1 || metaDataInterval <= 0)
		{
		...
		BufferedInputStream bufferedData = new BufferedInputStream(data, metaDataInterval * 2);
		try {
			...
			int bytesUntilInterval = metaDataInterval;
			byte sizeBuff;
			byte[] metaDataBuff = new byte[4080];
			//boolean recoveringFromError = false;
			//boolean closedData = false;
			while (isRunning && failures < 2 && !restart) {

				int timeouts = 0;
				while (isRunning
						&& (((readBytes = bufferedData.read(buff, 0, bytesUntilInterval)) != -1 && bytesUntilInterval != 0)
						|| bytesUntilInterval > 0) && timeouts < 100) {

					if(readBytes > 0 )
					{
					bytesUntilInterval -= readBytes;
					...
					{
						timeouts++;
						Log.d("StreamProxyTimeout", "incremented timeout counter");
					}
						
					//recoveringFromError = false;
				}
				if (isRunning && timeouts == 0) {
					sizeBuff = (byte) bufferedData.read();
					int metaDataSize = sizeBuff * 16;
					if (metaDataSize > 0) {
						bufferedData.read(metaDataBuff, 0, metaDataSize);
						String metaString = new String(metaDataBuff);
						if (metaString.contains("StreamTitle") == false) {
							// we have an error... need to resync
							...
							metaDataBuff = null;
							//buff = null;
							Log.d("StreamProxyError",
									"restarting data connection");

						} else {
							sendMetaString(metaString);
							failures = 0;
						}
					}
				...
				{
					bytesUntilInterval = metaDataInterval;
				}
			}
		} catch (Exception e) {
			...
			if (client != null && client.isConnected() && !restart) {
				client.close();
			}
	...
	private class IcyLineParser extends BasicLineParser {
		...
		public boolean hasProtocolVersion(CharArrayBuffer buffer,
			...
			return index + protolength <= buffer.length()
					&& buffer.substring(index, index + protolength).equals(
							ICY_PROTOCOL_NAME);
